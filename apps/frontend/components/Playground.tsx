...
import { VdfSolver, wasmUrl } from "@ucaptcha/js";

export function buildUrl(baseURL: string, path?: string, urlParams?: Record<string, string>): string {
...
export function Validating({ className }: { className?: string }) {
    ...
    function Status() {
        if (status === "success")
            return <span className="mt-4 font-bold text-green-600">Validation Result: Success</span>;
